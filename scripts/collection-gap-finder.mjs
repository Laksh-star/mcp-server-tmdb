...
async function main() {
  ...
  try {
    const rawText = await callToolText(client, "build_franchise_watch_order", {
      query: franchise,
      country,
      maxMovies,
    });
    const movies = parseReleaseOrder(rawText);
    const suggestedTitles = parseSuggestedTitles(rawText);
    const isWatched = watchedMatcher(watched);
    const watchedMovies = movies.filter(isWatched);
    const missingMovies = movies.filter((movie) => !isWatched(movie));
    const byTitle = new Map(missingMovies.map((movie) => [normalize(movie.title), movie]));
    const suggestedMissing = suggestedTitles.map((title) => byTitle.get(normalize(title))).filter(Boolean);
    const remaining = missingMovies.filter((movie) => !suggestedMissing.includes(movie));
